...
def truncate_summary(input_text, min_length, max_length, model, tokenizer):
    """
    It takes in a text, a minimum length, a maximum length, a model,
    and a tokenizer, and returns a summary
...
@click.option(
    "--url",
    type=str,
    default="https://en.wikipedia.org/wiki/Dimensionality_reduction",
)
def summarize(url):
    """
